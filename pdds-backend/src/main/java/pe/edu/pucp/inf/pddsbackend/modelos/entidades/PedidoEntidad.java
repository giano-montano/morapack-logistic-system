...
// Crea tablas de auditoría con cada registro histórico, esto con el
// BaseAuditable del AuditorAware nos dirá
// el histórico de qué cambio, cuándo, y quién sobre todo lo hizo.
// @Audited(targetAuditMode = NOT_AUDITED) // NOT_AUDITED PARA QUE NO SE LOQUEE
// CON EL ALMACÉN NO AUDITADO CON AUDITED (independiente del auditableBase creo)
...
public class PedidoEntidad extends BaseAuditable
{
    ...
    private Instant instanteMaximoParaEntregar; // no realmente necesario, pero para evitar
                                                // recomputar, nullable.

    @Column(nullable = false)
    private Boolean esIntercontinental;

    @ManyToOne(fetch = FetchType.LAZY, optional = true, cascade = CascadeType.PERSIST) // es
                                                                                       // nullable;
    // @JoinColumn(
    //
    // )
