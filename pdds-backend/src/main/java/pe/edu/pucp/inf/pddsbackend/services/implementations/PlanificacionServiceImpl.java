...
public class PlanificacionServiceImpl implements PlanificacionService
{
    ...
    // loggedHeuristicAlgorithmStrategy;
    ...
    @Override
    public ResultadoAlgoritmoDTO realizarPlanificacionConDatosDeBD(
            RealizarPlanificacionDTO parametros) throws Exception
    {
        Bitacora.escribir("========= REALIZA PLANIFICACION CON DATOS DE LA BD =========");

        EstadoGlobal estadoInicialAlgoritmo;
        EntradaProblemaPlanificacion entrada;
        ResultadoAlgoritmoDTO solucionAlgoritmo;

        estadoInicialAlgoritmo = this.obtenerDatosParaAlgoritmo(parametros, false);
        entrada = EntradaProblemaPlanificacion.builder()
                .estadoGlobal(estadoInicialAlgoritmo)
    ...
    public ResultadoAlgoritmoDTO realizarPlanificacionConEntrada(
        ...
        if (parametros.getUsarModoMock() != null && parametros.getUsarModoMock())
        {
            ...
            SalidaProblemaPlanificacion solucionPrueba = crearProgramacionesHardcodeadas(
                    dataEntradaAlgoritmo);
    ...
    private void escogerEstrategiaInicial(RealizarPlanificacionDTO parametros)
    {
        switch (parametros.getEstrategiaFija())
        {
            case AUTO -> this.estrategiaPlanificacion = new EstrategiaGraspHibrido();
            case PROFUNDA -> this.estrategiaPlanificacion = new EstrategiaGraspHibrido();
            case RAPIDA -> this.estrategiaPlanificacion = new EstrategiaGraspHibrido();
        }

        if (parametros.getSubCarpetaReportes() != null)
        {
            LoggingReport loggingReport = new LoggingReport();
            loggingReport.setDirectory(parametros.getSubCarpetaReportes());
            this.estrategiaPlanificacion.setLr(loggingReport);
        }
        else
        {
            this.estrategiaPlanificacion.getLr().limpiarDirectorio();
        }
...
//        Bitacora.escribir("EstrategiaPlanificacion inicializada");
            ...
            RealizarPlanificacionDTO params) throws Exception
    {
        ...
        // List<ProgramacionEntidad>enviosProgramados=
        // persistirSolucionYRetornarRutas(solucionAlgoritmo, params.getIdSimulacion());
            ...
            boolean incluirTodo)
    {
        ...
        Instant fechaPlanif = params.getInstanteActual() != null
                ? params.getInstanteActual()
                : Instant.now();
        ...
        System.out.println(
                "Obteniendo datos para algoritmo desde fecha pedidos: " + fechaInicioSimulacion);
        System.out.println("Fecha planificación: " + fechaPlanif);
        System.out.println("Fecha inicio vuelos (planif + 2h): " + fechaInicioVuelos);
        ...
        HashMap<Long, Vuelo> vuelos = obtenerVuelosParaAlgoritmo(fechaInicioVuelos,
                fechaMaxLLegadaVuelo, incluirTodo);
        HashMap<Long, Pedido> pedidos = obtenerPedidosParaAlgoritmo(fechaInicioSimulacion,
                fechaPlanif, incluirTodo);

        System.out.println("Total PEDIDOS obtenidos: " + pedidos.size());
        System.out.println("Total VUELOS obtenidos: " + vuelos.size());
         ...
         * System.out.println("   - Vuelos desde: " + fechaInicioVuelos +
         * " (planif + 2h)"); System.out.println("   - Vuelos hasta: " +
         * fechaMaxLLegadaVuelo + " (planif + 3 días)");
         */
        System.out.println("===============================================\n");
            ...
            boolean incluirTodo)
    {
        ...
        else
        {
            pedidos = pedidoRepository
                    .listarPedidosNoAtendidosCompletamenteYNoDeAlmacenesInfinitosEntreMedio(
                            fechaSimulacionInicio, fechaPlanif);
    ...
    /**
     * Construye un mapa idAlmacen -> Almacen (versión simple: consulta por cada
     * almacén las listas de ids). Se puede BATCHEAR para más eficiencia
     */
            ...
            boolean incluirTodo)
    {
        ...
        {
            vuelos = vueloRepository
                    .findByActivoTrueAndFechaHoraInicioUtcAfterAndFechaHoraFinUtcBefore(fechaInicio,
                            fechaMaxLlegadaVuelos);
     ...
     * una Planificacion nueva - una ProgramacionEntidad por cada Programacion
     * (vinculada al PedidoEntidad) - una ProgramacionXVuelo por cada vuelo en la
     * ruta (en el mismo orden)
     *
     * Además marca el PedidoEntidad como PROGRAMADO y actualiza la capacidad
     * ocupada del VueloEntidad incrementándola por la cantidad programada en la
     * ruta (ver notas).
     *
     * La operación es transaccional: ante cualquier error se hace rollback.
     */
    @Transactional
    public List<ProgramacionEntidad> persistirSolucionYRetornarRutas(ResultadoAlgoritmoDTO solucion,
            Long idSimulacion)
    {
        // if (solucion == null ||
        // solucion.getRutasProgramadasParaSatisfacerTodoPedido() == null) {
        // return List.of();
        // }
        // Simulacion simulacion=null;
        // if(idSimulacion != null) { simulacion =
        // simulacionRepository.getReferenceById(idSimulacion);}
        ...
        // // AUNQUE ESTÉ COLAPSADO LO GUARDAMOS, PARA CURIOSEAR; PERO TENERLO EN
        // CUENTA!!!
        // List<ProgramacionEntidad> rutasPersistidas = new ArrayList<>();
        //
        // // iterar rutas generadas por el algoritmo
        // for (Programacion rutaAlgo :
        // solucion.getRutasProgramadasParaSatisfacerTodoPedido()) {
        //
        ...
        // ProgramacionEntidad rutaGuardada =
        // rutaProgramadaRepository.save(rutaEntidad);
        //
        ...
        // .orElseThrow(() -> new EntityNotFoundException("VueloEntidad no encontrado
        // id=" + idVuelo));
        //
        ...
        // int nuevaOcupada = Math.addExact(vuelo.getInventario().size(), cantidad);
        // if (nuevaOcupada > vuelo.getCapacidadMaxima()) {
        // // si supera capacidad, lanzamos excepción para abortar toda la transacción
        // throw new IllegalStateException("Capacidad de vuelo excedida (vueloId=" +
        // idVuelo +
        // " capacidadMaxima=" + vuelo.getCapacidadMaxima() +
        // " ocupadoAntes=" + vuelo.getInventario().size() +
    ...
    @Transactional(readOnly = true)
    protected PlanificacionResponseDTO mapearSolucionAResponse(
            ResultadoAlgoritmoDTO resultadoAlgoritmoDTO)
    {
//        SalidaProblemaPlanificacion solucion = resultadoAlgoritmoDTO.salida();
//        if (solucion == null || solucion.getProgramaciones() == null
//                || solucion.getProgramaciones().isEmpty())
//        {
//            return new PlanificacionResponseDTO(null, null, false, null, null, 0L,
...
//            if (p == null)
//                continue;
//            pedidoIds.add(p.getPedido().getId());
//            if (p.getRuta() != null)
//            {
//                vueloIds.addAll(p.getRuta());
//            }
...
//        if (!vueloIds.isEmpty())
//        {
//            // vueloRepository.findAllById(vueloIds).forEach(v -> vuelosMap.put(v.getId(),
//            // v));
//        }
//        vueloRepository.findAll().forEach(v -> vuelosMap.put(v.getId(), v));
//
//        // System.out.println("vuelosMap: "+vuelosMap.keySet()+" :
//        // "+vuelosMap.values());
//        // Productos: intento de batch fetch; si tu repo no tiene findAllByUuidIn, crea
//        // un método o cae a per-item abajo
//        Map<UUID, ProductoEntidad> productosMap = new HashMap<>();
//        if (!productoUuids.isEmpty())
//        {
//            try
//            {
//                // suponiendo que existe: List<ProductoEntidad>
//                // findAllByUuidIn(Collection<UUID>)
...
//                {
//                    productoRepository.findByUuid(uuid)
//                            .ifPresent(p -> productosMap.put(p.getUuid(), p));
//                }
//            }
//        }
//
...
//        {
//            if (pe != null && pe.getAlmacenDestino() != null)
//                almacenIds.add(pe.getAlmacenDestino().getId());
//        }
...
//                almacenIds.add(v.getAlmacenOrigen().getId());
//            if (v.getAlmacenDestino() != null)
//                almacenIds.add(v.getAlmacenDestino().getId());
//        }
...
//                Bitacora.escribir("mapearSolucionAResponse: pedido no encontrado id="
...
//
//            // ProductoEntidad: chequear en el mapa; si falta, crear/obtener con la función
//            // existente
...
//                {
//                    Bitacora.escribir(
//                            "mapearSolucionAResponse: producto no encontrado y no pudo crearse uuid="
//                                    + programacion.getUuidProducto());
//                    continue;
//                }
//            }
//
...
//                    almacenDto);
//
//            // Vuelos asociados: tomamos desde vuelosMap usando ids de la ruta (mantenemos
//            // orden)
...
//                        if (ve == null)
//                        {
//                            Bitacora.escribir("mapearSolucionAResponse: vuelo no encontrado id="
//                                    + vid + " (se omite)");
//                            continue;
//                        }
//                        else
...
//
//                    vDto.setCodigoAeropuertoOrigenEn4Siglas(
//                            origen != null ? origen.getCodigoAeropuertoEn4Letras() : null);
//                    vDto.setCodigoAeropuertoDestinoEn4Siglas(
//                            destino != null ? destino.getCodigoAeropuertoEn4Letras() : null);
//
//                    vDto.setCiudadOrigenEn4Siglas(
//                            origen != null ? origen.getCodigoCiudadEn4Letras() : null);
//                    vDto.setCiudadDestinoEn4Siglas(
//                            destino != null ? destino.getCodigoCiudadEn4Letras() : null);
//
//                    vDto.setOrden(orden++);
//                    vuelosDto.add(vDto);
//                }
...
//                solucion.getError());
            ...
            List<VueloEntidad> vuelos)
    {
        ...
        return ProductoEntidad.builder()
                .uuid(programacion.getProducto().getId())
                .existe(false)
    ...
    /**
     * 🎭 MAPEO MOCK: Convierte solución a response DTO SIN tocar la BD Solo usa
     * datos del dominio (Programacion) para generar DTOs ficticios Útil para
     * testing cuando no se quiere persistir nada
     */
    protected PlanificacionResponseDTO mapearSolucionMockAResponse(
            ResultadoAlgoritmoDTO resultadoAlgoritmoDTO)
    {
        SalidaProblemaPlanificacion solucion = resultadoAlgoritmoDTO.salida();
    ...
    /**
     * 🧪 MÉTODO PARA TESTING: Crea programaciones HARDCODEADAS para probar la
     * simulación
     *
     * IMPORTANTE: Debes ajustar los IDs según tu base de datos: - idPedido: ID de
     * un pedido existente en tu BD - uuidProducto: UUID de un producto existente -
     * idsVuelo: IDs de vuelos existentes en tu BD
     *
     * Para encontrar IDs válidos, ejecuta estas queries: SELECT id FROM pedidos
     * LIMIT 3; SELECT uuid FROM productos LIMIT 1; SELECT id FROM vuelos LIMIT 5;
     */
    private SalidaProblemaPlanificacion crearProgramacionesHardcodeadas(
            EntradaProblemaPlanificacion datos)
    {
