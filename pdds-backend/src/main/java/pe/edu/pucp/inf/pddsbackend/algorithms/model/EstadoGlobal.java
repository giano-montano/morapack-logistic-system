...
public class EstadoGlobal implements Serializable {
    ...
    @NotNull
    private HashMap<UUID, Producto> productos; // Algoritmo añadirá
...
//            if(loggingReport!=null)
     ...
     */
//    public AlmacenParaAlgoritmo obtenerAlmacenEnInstante(Almacen alm, Instant instante){ //usamos las rutas programadas hasta ahora
//        Almacen almacenSimuladoHastaInstante = new Almacen(alm);
//        long idAlmacenSimulado = alm.getId();
//
//        for(Programacion prog : programaciones){
//            List<VueloParaAlgoritmo> vuelitos = obtenerVariosVuelosPorIds(prog.getIdsVueloRuta());
//            Pro
    ...
    public synchronized void eliminarRutaSolucion(RutaProgramadaParaAlgoritmo r) {
        ...
        if (!this.rutasSolucionQueGeneraAlgoritmo.contains(r)) {
            ...
            if (this.loggingReport != null) {
                this.Bitacora.escribir("Intentaron eliminar una ruta que no existe en la mesa. Ignorando.");
            }
            return;
        }
        ...
        try {
            ...
            // 4) (Opcional) actualizar índices auxiliares si los mantienes, p.ej. idsRutasPorAlmacenDestino
            ...
            if (this.loggingReport != null) {
                this.Bitacora.escribir("Ruta eliminada correctamente. pedidoId=" + idPedido + " cantidad=" + cantidad);
            }
                ...
                try {
                    ...
                    if (vueloRollback != null) {
                        ...
                        if (!reocupo && this.loggingReport != null) {
                            this.Bitacora.escribir("ROLLBACK: No se pudo re-ocupar vuelo id=" + idVueloRollback +
                    ...
                    if (this.loggingReport != null) {
                        this.Bitacora.escribir("ROLLBACK ERROR en vuelo id=" + idVueloRollback + ": " + inner.getMessage());
                    }
                }
            }
            ...
            if (this.loggingReport != null) {
                this.Bitacora.escribir("Error al eliminar ruta; rollback parcial efectuado. Error: " + ex.getMessage());
            }
            throw ex;
        }
    }
...
//            int programados = p.getCantidadProductosProgramados() == null ? 0 : p.getCantidadProductosProgramados();
    ...
    public List<RutaProgramadaParaAlgoritmo> generarTodasRutasPosiblesATodosDestinos() {
        ...
        for (Long destId : idsDestinos) {
            ...
            for (AlmacenParaAlgoritmo origen : origenes) {
                ...
                Queue<List<VueloParaAlgoritmo>> queueVuelos = new ArrayDeque<>();
                ...
                for (VueloParaAlgoritmo v : iniciales) {
                    ...
                    path.add(v);
                    queueVuelos.add(path);
                }

                int rutasPorOrigen = 0;
                while (!queueVuelos.isEmpty() && rutasPorOrigen < MAX_RUTAS_POR_ORIGEN
                        && rutasEncontradasParaDestino < MAX_RUTAS_POR_DESTINO) {
                    List<VueloParaAlgoritmo> path = queueVuelos.poll();
        ...
        for ( RutaProgramadaParaAlgoritmo ruta : resultado) {
            ...
            for(Long idV : ruta.getIdsVuelosEnOrden()) {
                Bitacora.escribir( "   Vuelo:"+ vuelos.get(idV));
            }
        }
        return resultado;
    ...
    public boolean esFactibleLlevarPedidoEnRuta(Long idPedido, RutaProgramadaParaAlgoritmo rutaProspecto) {
        ...
        if (idPedido == null || rutaProspecto == null || rutaProspecto.getIdsVuelosEnOrden() == null
            ...
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: parámetros inválidos.");
            return false;
        }
        ...
        PedidoParaAlgoritmo pedido = this.pedidos.get(idPedido);
        if (pedido == null) {
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: pedido no existe id=" + idPedido);
            return false;
        }
...
//        int remaining = Math.max(0, total - entregados - programados);
        if (pedido.getCantidadRestanteDeEntregaYProgram() <= 0) { // ya se validó que es pendiente.
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: pedido id=" + idPedido + " sin remanente.");
            return false;
        }
        ...
        if (vuelosRuta.size() != idsVuelos.size()) {
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: falta info de algún vuelo en la ruta prospecto.");
            return false;
        }
        ...
        if (!Objects.equals(ultimoVuelo.getIdAlmacenDestino(), pedido.getIdAlmacenDestino())) {
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: ruta no termina en el almacén destino del pedido.");
            return false;
        }
        ...
        for (VueloParaAlgoritmo vuelo : vuelosRuta) {
            ...
            if (prev != null) {
                if (!Objects.equals(prev.getIdAlmacenDestino(), vuelo.getIdAlmacenOrigen())) {
                    if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: ruta desconectada entre vuelos.");
                    return false;
                }
                // orden temporal
                if (vuelo.getInicio().isBefore(prev.getFin())) {
                    if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: solapamiento temporal entre tramos.");
                    return false;
                }
            }
            ...
            if (capVuelo <= 0) {
                if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: vuelo id=" + vuelo.getId() + " sin capacidad.");
                return false;
            }
            ...
            if (almOrigen == null) {
                if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: almacen origen no encontrado id=" + vuelo.getIdAlmacenOrigen());
                return false;
            }
            String keyOrigen = almOrigen.getId() + "|" + vuelo.getInicio().toString();
            ...
            if (simulOrigen.getCapacidadSinOcupar() <= 0) {
                if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: almacen origen id=" + almOrigen.getId() + " sin stock en instante salida.");
                return false;
            }
            ...
            if (almDestino == null) {
                if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: almacen destino no encontrado id=" + vuelo.getIdAlmacenDestino());
                return false;
            }
            String keyDestino = almDestino.getId() + "|" + vuelo.getFin().toString();
            ...
            if (simulDestino.getCapacidadSinOcupar() <= 0) {
                if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: almacen destino id=" + almDestino.getId() + " sin espacio a la llegada.");
                return false;
            }
        ...
        if (llegadaFinal == null) {
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: último vuelo sin instante de llegada.");
            return false;
        }
        ...
        if (deadline != null && pickup.isAfter(deadline)) {
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: pickup " + pickup + " posterior al deadline " + deadline);
            return false;
        }
        ...
        if (asignable <= 0) {
            if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: ninguna unidad asignable (remaining=" + pedido.getCantidadRestanteDeEntregaYProgram() + ", minCapVuelos=" + minimaCapacidadVuelos + ").");
            return false;
        }

        // pasa todos los chequeos conservadores: al menos 1 unidad asignable
        if (this.loggingReport != null) this.Bitacora.escribir("esFactibleLlevarPedidoEnRuta: pedido id=" + idPedido + " factible en la ruta prospecto. asignable>=1");
        return true;
    }
    ...
    /*false si no pudo; true si es que sí y muta el pedido de la ruta*/
    public boolean cumpleConPlazoEntregaYLoModifica(PedidoParaAlgoritmo pedido, RutaProgramadaParaAlgoritmo rutaProspecto) {
        // NUEVO: PARA VALIDACIÓN PARA EL INSTANTE MÁXIMO DE ENTREGA Y SU CAMBIO EN CASO SEA INTERCONTINENTAL
        // O CONTINENTAL AHORA
        AlmacenParaAlgoritmo almDestinoFinalDelPedido = almacenes.get(pedido.getIdAlmacenDestino());
        VueloParaAlgoritmo ultimoVueloRuta = vuelos.get(rutaProspecto.getIdsVuelosEnOrden().getLast());
        Instant posibleInstanteMaxEntrega = verificarYDarPosibleInstanteMaxEntrega(pedido,almDestinoFinalDelPedido,rutaProspecto);

        if(!ultimoVueloRuta.getFin().isAfter(
                Utils.restarHorasAInstante(posibleInstanteMaxEntrega,Constantes.HORAS_ESPERA_PARA_RECOJO))){
            // si el último vuelo no llega después del inst max de entrega menos 2 horas
            pedido.setInstanteMaximoParaEntregar(posibleInstanteMaxEntrega);
            return true;
        }else{
            return false;
        }
    ...
    private Instant verificarYDarPosibleInstanteMaxEntrega(PedidoParaAlgoritmo pedido,
                                                           AlmacenParaAlgoritmo almDestinoFinalPedido,
                                                           RutaProgramadaParaAlgoritmo rutaProspecto) {
        //ojo: no me importa si en medio de cambian de continente o no; todo es en base al origen del prod
        if(pedido.getCantidadProductosEntregados()>0){ // Si el pedido YA TIENE entregados
            List<ProductoParaAlgoritmo> prodsExistentesDePedido = pedido.getIdsProductosYaEntregados().stream().map(
                    aLong -> productosExistentes.get(aLong)
            ).toList();
            boolean hayAlMenos1ProdEntregadoIntercont = prodsExistentesDePedido.stream().anyMatch( //modulable/encapsulable?
                    productoParaAlgoritmo -> {
                        AlmacenParaAlgoritmo almOrigProd = almacenes.get(productoParaAlgoritmo.getIdAlmacenInfinitoOrigen());
                        return ! almOrigProd.getContinente().equals(almDestinoFinalPedido.getContinente());
                    } );
            if(hayAlMenos1ProdEntregadoIntercont){  // Si de los entregados, al menos uno es intercontinental
                return pedido.getInstanteMaximoParaEntregar()!=null?
                        pedido.getInstanteMaximoParaEntregar():
                        Utils.aumentarDiasAInstante(pedido.getInstanteRegistro(),Constantes.DIAS_CONTINENTAL);
            }else{ // Si aún no hay ningún entregado intercontinental
                if(rutaProspecto.isEsIntercontinentalPorVuelos()){ // la ruta propone un intercontinental
                    return Utils.aumentarDiasAInstante(pedido.getInstanteRegistro(), Constantes.DIAS_INTERCONTINENTAL);
                }else{ // la ruta propone un continental
                    return pedido.getInstanteMaximoParaEntregar()!=null?
                            pedido.getInstanteMaximoParaEntregar():
                            Utils.aumentarDiasAInstante(pedido.getInstanteRegistro(),Constantes.DIAS_CONTINENTAL);
                }
            }
        }else{ // Si el pedido AÚN NO tiene entregados
            if(rutaProspecto.isEsIntercontinentalPorVuelos()){ // la ruta propone un intercontinental
                return Utils.aumentarDiasAInstante(pedido.getInstanteRegistro(), Constantes.DIAS_INTERCONTINENTAL);
            }else{ // la ruta propone un continental
                return Utils.aumentarDiasAInstante(pedido.getInstanteRegistro(),Constantes.DIAS_CONTINENTAL);
            }
        }
    }

    private int obtenerMaxAsignableEntreMedioVuelosYRecojo(int asignable, LinkedList<VueloParaAlgoritmo> vuelosRuta) {
        if (asignable > 0) {
...
//        List<PedidoParaAlgoritmo> nuevaLista = new ArrayList<>();
        for (PedidoParaAlgoritmo p : pedidos.values()) {
            ...
            if (remaining <= 0) {
                Bitacora.escribir("remover: pedido id=" + p.getId() + " removido (satisfecho).");
                continue;
            }
            ...
            if (!factible) {
                Bitacora.escribir("remover: pedido id=" + p.getId() + " no factible para la ruta actual -> se remueve del pool local");
                continue;
            }
    ...
    public Integer  obtenerCapacidadMaxParaTodosVuelosEnRuta(RutaProgramadaParaAlgoritmo rutaSeleccionada){
        ...
        return rutaSeleccionada.getIdsVuelosEnOrden().stream()
                .mapToInt(vId -> {
                    ...
                    if(loggingReport!=null) Bitacora.escribir(
...
//    }
    ...
    public EstadoGlobal crearSnapshot() {
...
//        List<UUID> listaProdsAlmacenOrigen = productos.values().stream()
//                        .filter(producto -> producto.getIdAlmacenActual()==almacenOrigen.getId())
//                .map(Producto::getUuid)
//                .toList();
        // vvv tener los ids prods en almacén y poderlos simular en el tiempo GODDDDD
    ...
    public Almacen getAlmacenEnInstante(Almacen almacen, Instant instante) { // nuevo
        ...
        for(Programacion programacionProd : programaciones){
            List<VueloParaAlgoritmo> vuelitos = obtenerVariosVuelosPorIds(programacionProd.getIdsVueloRuta());
//            int cantProdsRuta = 1;
